...
const { fetchActivities, repackageToMBZ, updateActivities } = require('./xmlReader')
const MoodleQuiz = require('../app/models/moodle_quiz')

var path = "data/backup-moodle2-course-17014-s20222-log210-99-20220619-1506-nu.mbz"
var new_path = "tmp/backup-moodle2-course-17014-s20222-log210-99-20220619-1506-nu/"
...
const test = async function () {
    ...
    var newTimes = getListModifiedTimes(calendarActivities, DSLParser.parse(string)[1])
    console.log(newTimes)

    var activities = fetchActivities("tmp/backup-moodle2-course-17014-s20222-log210-99-20220619-1506-nu")
    ...
    updateActivities("tmp/backup-moodle2-course-17014-s20222-log210-99-20220619-1506-nu/", activities)
    repackageToMBZ("tmp/backup-moodle2-course-17014-s20222-log210-99-20220619-1506-nu/")
}
// test()
...
const getListModifiedTimes = function (calendarActivities, pegObj) {
    ...
    for (const obj of pegObj) {
        ...
        var dateObj = {}
        if(activity.includes("Quiz")) dateObj = getNewQuizDates(obj, calendarActivities)
        if(activity.includes("Homework")) dateObj = getNewHomeworkDates(obj, calendarActivities)
        if(activity.includes("Exam")) dateObj = getNewExamDates(obj, calendarActivities)
...
const getNewQuizDates = function (obj, calendarActivities) {
    ...
    var openCalendarAct = getCalendarActivity(calendarActivities, openActivityName, openActivityNumber)
    ...
    // Get correct calendar activity for oepn date
    var closeCalendarAct = getCalendarActivity(calendarActivities, closeActivityName, closeActivityNumber)
...
const getNewHomeworkDates = function (obj, calendarActivities){
    ...
    // Get correct calendar activity for oepn date
    var dueCalendarAct = getCalendarActivity(calendarActivities, dueActivityName, dueActivityNumber)
    ...
    // Get correct calendar activity for oepn date
    var cutoffCalendarAct = getCalendarActivity(calendarActivities, cutoffActivityName, cutoffActivityNumber)
...
const getCalendarActivity = function (calendarActivities, activityName, activityNumber) {
    var activity
    switch (activityName) {
        case "Seminar":
            activity = calendarActivities.seminars[Number.parseInt(activityNumber) - 1]
            break;
        case "Laboratory":
            activity = calendarActivities.laboratories[Number.parseInt(activityNumber) - 1]
            break;
        case "Practicum":
            activity = calendarActivities.practicums[Number.parseInt(activityNumber) - 1]
            break;
        default:
            activity = undefined
            break;
